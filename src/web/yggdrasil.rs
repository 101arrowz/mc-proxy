use crate::{connection::{error::Error as ConnectionError, packets::login::{LoginCredentials, Authenticator}}, protocol::types::{UUID, serde_raw_uuid}};
...
impl Authentication<'_> {
    pub fn new<'a>(client_token: Option<&'a str>, access_token: Option<Cow<'a, str>>, client: Option<Client>) -> Authentication<'a> {
        Authentication {
            client_token,
            access_token,
    ...
    pub async fn authenticate(&mut self, username: &str, password: &str) -> Result<AuthenticationResponse<'_>, WebError> {
    ...
    pub async fn get_access_token(&mut self) -> Result<Option<&str>, WebError> {
        if let Some(access_token) = self.access_token.take() {
            if self.client.post("https://authserver.mojang.com/validate")
                .json(&ValidationRefreshRequest {
                    client_token: self.client_token,
                ...
                .status() == StatusCode::NO_CONTENT
            {
                ...
                self.refresh_access_token().await.map(|resp| Some(resp.access_token))
            }
        } else {
            Ok(None)
        }
...
impl Authenticator for OnlineMode<'_> {
    type CredentialsOutput<'a> = impl Future<Output = Result<LoginCredentials<'a>, ConnectionError>> + 'a;
